...
public class MainActivity extends AppCompatActivity {
    ...
    private static void printTopFive(String[] fiveHighestWords, int[] fiveHighestFrequencies, int tCnt) {
        for (int i = 0; i < fiveHighestWords.length; ++i) {
            if (i == 0) {
                t += (i+1 + ") " + fiveHighestWords[i] + ": " + fiveHighestFrequencies[i]);
                t += "\n";
            }
            else {
                if (fiveHighestFrequencies[i] == fiveHighestFrequencies[i-1]) {
                    tCnt++;
                    t += (i+1-tCnt + ") " + fiveHighestWords[i] + ": " + fiveHighestFrequencies[i]);
                    t += "\n";
                } else {
                    tCnt = 0;
                    t += (i+1 + ") " + fiveHighestWords[i] + ": " + fiveHighestFrequencies[i]);
